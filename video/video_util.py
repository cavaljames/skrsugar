...
class ScreenShot:
    @staticmethod
    def screen_shot(video_dir='', screen_shot_path='/Users/sugar/PycharmProjects/skrsugar/screenshots', num_capture=30, filter_size=150 * MB):
        for fpathe, dirs, fs in os.walk(video_dir):
            for f in fs:
                video_path = os.path.join(fpathe, f)
                os.stat(video_path)
                video_file_name = video_path.split('/')[-1]
                if (not video_file_name.lower().endswith('.mp4')) or (os.stat(video_path).st_size < filter_size):
                    continue
